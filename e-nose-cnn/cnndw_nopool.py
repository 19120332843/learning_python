...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1d = nn.Conv2d(in_channels = 10, out_channels = 10, kernel_size = (1, 4), stride = (1, 2), groups = 10)#(120 - 4)/2 + 1 = 59
        self.conv1p = nn.Conv2d(in_channels = 10, out_channels = 6, kernel_size = 1, stride = 1, groups = 1)
        self.hswish1 = hswish()
        self.conv2d = nn.Conv2d(in_channels = 6, out_channels = 6, kernel_size = (1, 3), stride = (1, 2), groups = 6)#(59 - 3)/2 + 1 = 29
...
if __name__ == '__main__':
    ...
    for epoch in range(300):
        ...
        if (sum / total_train > 0.90) :
            ...
            optimizer = optim.SGD(cnn.parameters(), lr=lrr/10/10, momentum=0.8/2/2)#momentum=0
    ...
    #test
        ...
        if(max < sum/total):
            max = sum/total
            maxepoch = epoch + 1
            torch.save(cnn, './net/mobilenet_nopool.pkl')
